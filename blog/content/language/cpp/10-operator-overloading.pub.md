---
title: "[C++ 학습 노트 10] 연산자 오버로딩"
description: "Point 클래스와 직접 만든 문자열 클래스로 산술, 비교, 대입, 스트림 삽입 연산자를 오버로딩하는 패턴을 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  - C++
  - OperatorOverloading
summary: "멤버 함수와 비멤버 함수로 연산자를 오버로딩하는 차이, operator<<가 멤버 함수가 될 수 없는 이유, 직접 만든 문자열 클래스로 깊은 복사를 복습합니다."
---

사용자 정의 타입도 내장 타입처럼 `+`, `==`, `<<` 같은 연산자를 자연스럽게 쓸 수 있게 해주는 것이 연산자 오버로딩입니다. 이 글에서는 간단한 `Point` 클래스로 대표적인 오버로딩 패턴을 살펴보고, 09장에서 다룬 깊은 복사를 직접 만든 문자열 클래스에 적용해봅니다.
...
이전 글: [09. 복사와 이동 의미론](./09-copy-and-move-semantics.pub.md)

다음 글: [11. 상속](./11-inheritance.pub.md)
