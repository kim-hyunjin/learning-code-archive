---
title: "[C++ 학습 노트 09] 복사와 이동 의미론"
description: "얕은 복사가 이중 해제로 이어지는 이유와 깊은 복사, Rule of Three, C++11 이동 생성자로 복사 비용을 줄이는 법을 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  ...
  - MoveSemantics
  - CopySemantics
summary: "포인터 멤버를 가진 클래스에서 얕은 복사가 이중 해제로 이어지는 이유, 깊은 복사와 Rule of Three, rvalue 참조 기반 이동 생성자로 복사 비용을 없애는 법을 정리합니다."
---

07장에서 다룬 `new`/`delete`를 클래스 멤버로 들고 있으면, 그 클래스를 복사할 때 무슨 일이 일어나는지가 중요해집니다. 이 글에서는 컴파일러가 자동으로 만들어주는 얕은 복사가 왜 위험한지, 이를 해결하는 깊은 복사와 Rule of Three, 그리고 복사 비용 자체를 없애는 C++11의 이동 의미론을 다룹니다.
...
이전 글: [08. 클래스와 객체](./08-classes-and-objects.pub.md)

다음 글: [10. 연산자 오버로딩](./10-operator-overloading.pub.md)
