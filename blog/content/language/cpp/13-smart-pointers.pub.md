---
title: "[C++ 학습 노트 13] 스마트 포인터"
description: "unique_ptr, shared_ptr, weak_ptr로 RAII 패턴을 적용해 메모리 누수와 이중 해제를 막는 법과 커스텀 삭제자를 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  ...
  - SmartPointers
  - RAII
summary: "unique_ptr로 단독 소유를, shared_ptr로 공유 소유를 표현하는 법과 weak_ptr로 순환 참조를 끊는 법, make_unique/make_shared를 우선해야 하는 이유를 정리합니다."
---

07장에서 본 것처럼 `new`/`delete`를 직접 다루면 해제를 빼먹거나 이중 해제하는 실수가 생기기 쉽습니다. 이 글에서는 객체가 스코프를 벗어날 때 자동으로 자원을 해제하는 RAII 패턴을 포인터에 적용한 스마트 포인터 세 종류 — `unique_ptr`, `shared_ptr`, `weak_ptr` — 를 다룹니다.
...
이전 글: [12. 다형성](./12-polymorphism.pub.md)

다음 글: [14. 예외 처리](./14-exception-handling.pub.md)
