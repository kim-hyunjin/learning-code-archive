---
title: "[C++ 학습 노트 11] 상속"
description: "베이스/파생 클래스와 protected 접근 제어자, 베이스 클래스 초기화 순서, 메서드 재정의가 다형성 없이는 정적 바인딩된다는 점을 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  - C++
  - Inheritance
summary: "is-a 관계에서만 상속을 써야 하는 이유, 베이스 클래스가 파생 클래스보다 먼저 생성되고 나중에 소멸하는 순서, 가상 함수가 아닌 재정의가 정적 바인딩되는 문제를 정리합니다."
---

상속은 "A는 B의 한 종류다"라는 is-a 관계를 표현하는 도구입니다. 이 글에서는 베이스 클래스를 초기화하는 순서, `protected` 접근 제어자의 역할을 다루고, 마지막에는 일반 멤버 함수 재정의가 왜 다음 글(12장 다형성)에서 배울 가상 함수 없이는 기대한 대로 동작하지 않는지를 보여줍니다.
...
이전 글: [10. 연산자 오버로딩](./10-operator-overloading.pub.md)

다음 글: [12. 다형성](./12-polymorphism.pub.md)
