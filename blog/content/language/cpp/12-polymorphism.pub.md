---
title: "[C++ 학습 노트 12] 다형성"
description: "virtual 키워드로 동적 바인딩을 얻는 법, override/final의 안전성, 순수 가상 함수로 인터페이스를 만드는 법, 가상 소멸자가 필요한 이유를 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  ...
  - Polymorphism
  - VirtualFunctions
summary: "virtual로 정적 바인딩 문제를 해결하는 법, 순수 가상 함수로 추상 클래스를 만드는 법, 다형적으로 쓰일 베이스 클래스에 가상 소멸자가 반드시 필요한 이유를 정리합니다."
---

11장 끝에서 본 것처럼 일반 멤버 함수 재정의는 포인터의 선언된 타입을 기준으로 동작합니다. 이 글에서는 `virtual` 키워드로 이 문제를 해결하는 다형성의 핵심 메커니즘과, 추상 클래스·가상 소멸자처럼 다형성을 안전하게 쓰기 위해 함께 알아야 할 것들을 다룹니다.
...
이전 글: [11. 상속](./11-inheritance.pub.md)

다음 글: [13. 스마트 포인터](./13-smart-pointers.pub.md)
