---
title: "[C++ 학습 노트 15] 파일 입출력과 스트림"
description: "ofstream/ifstream으로 파일을 읽고 쓰는 법, stringstream으로 문자열을 파싱하는 법, iomanip 조작자로 출력 형식을 다루는 법을 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  ...
  - FileIO
  - Streams
summary: "std::cin/cout과 같은 <<, >> 인터페이스로 파일과 문자열을 다루는 법, while (in >> ...) 패턴이 EOF에서 자연스럽게 끝나는 이유, setw/setprecision 조작자를 정리합니다."
---

`std::cin`/`std::cout`이 콘솔과 주고받던 `<<`, `>>` 인터페이스는 파일과 문자열에도 그대로 적용됩니다. 이 글에서는 `<fstream>`으로 파일을 읽고 쓰는 법과 `<sstream>`으로 문자열을 스트림처럼 파싱하는 법, 그리고 `<iomanip>`의 출력 조작자를 다룹니다.
...
이전 글: [14. 예외 처리](./14-exception-handling.pub.md)

다음 글: [16. 템플릿](./16-templates.pub.md)
