---
title: "[C++ 학습 노트 16] 템플릿"
description: "함수 템플릿과 클래스 템플릿으로 타입을 매개변수화하는 법, 템플릿 인스턴스화 원리, 구현을 헤더에 두어야 하는 이유를 정리합니다."
date: 2026-09-09
category: "Language"
categories:
  - Language
...
tags:
  ...
  - Templates
  - Generics
summary: "타입 추론이 모호할 때 명시적으로 타입을 지정하는 법, 값을 받는 non-type 템플릿 매개변수, 템플릿 구현을 .cpp가 아닌 헤더에 두어야 하는 이유를 정리합니다."
---

`int`용, `double`용, `std::string`용 함수를 매번 따로 작성하는 대신, 템플릿은 타입을 매개변수로 받는 함수나 클래스를 작성할 수 있게 해줍니다. 이 글에서는 함수 템플릿과 클래스 템플릿의 기본 문법을 살펴보고, 표준 라이브러리 컨테이너들이 왜 클래스 템플릿으로 만들어져 있는지에 대한 감을 잡습니다.
...
이전 글: [15. 파일 입출력과 스트림](./15-file-io-and-streams.pub.md)

다음 글: [17. STL 컨테이너](./17-stl-containers.pub.md)
